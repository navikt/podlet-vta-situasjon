...
describe("tester at meldekort-hent-frister fungerer", () => {
  ...
  test("den returnerer 6 dager til inaktivering og 3 dager etter periodeslutt for 2020-12-15", () => {
    ...
    expect(dagerFraPeriodeSlutt).toBe(3);
  });

  test("den returnerer 6 dager til inaktivering og 3 dager etter periodeslutt for 2020-12-29", () => {
  ...
  test("den returnerer 0 til inaktivering og 9 dager etter periodeslutt for 2021-01-04", () => {
    const iDag = new Date("2021-01-04");
    const dager = hentMeldekortFrister(iDag, meldekortHistore);
    const { dagerTilInaktivering, dagerFraPeriodeSlutt } = dager;
    expect(dagerTilInaktivering).toBe(0);
    expect(dagerFraPeriodeSlutt).toBe(9);
  });
});
